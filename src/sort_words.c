...
int main(int argc, char **av)
{
    ...
    if (d == NULL)
    {
        fprintf(stderr, "Could not open input dictionary file\n");
        exit(-1);
    }
    ...
    FILE *sorted_dictionary = fopen(av[2], "wt");
    if (sorted_dictionary == NULL)
    {
        fprintf(stderr, "Could not open output dictionary file\n");
        exit(-1);
    }
