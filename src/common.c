...
uint32_t estimate_seq_size(FILE *f)
{
    ...
    if (f == NULL)
    {
        fprintf(stderr, "File is null. Check inputs.\n");
        exit(-1);
    }
...
char *allocate_memory_for_sequence(FILE *f)
{
    ...
    if (l == 0)
    {
        fprintf(stderr, "Sequence is 0-sized. Check inputs.\n");
        exit(-1);
    }
    char *s = NULL;
...
Word *load_words(FILE *f, uint32_t *n)
{
    ...
    if (words == NULL)
    {
        fprintf(stderr, "No memory for allocating words.\n");
        exit(-1);
    }
    ...
    while (!feof(f))
    {
        ...
        if (fgets(buffer, MAX_LINE, f) != NULL)
        {
            ...
            if (i % BATCH_SIZE == 0)
                words = (Word *)realloc(words, (i / BATCH_SIZE + 1) * BATCH_SIZE * sizeof(Word));
        }
    }
...
void quick_sort_words_inplace(Word *words, uint32_t x, uint32_t y, uint32_t k_size)
{
    ...
    do
    {
        while (strncmp(pivot.kmer, words[x1].kmer, k_size) > 0)
            x1++;
        while (strncmp(pivot.kmer, words[y1].kmer, k_size) < 0)
            y1--;
    ...
    if (x < y1)
        quick_sort_words_inplace(words, x, y1, k_size);
    if (x1 < y)
        quick_sort_words_inplace(words, x1, y, k_size);
}
